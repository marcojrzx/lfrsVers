...
if ($_POST['status']) {
	...
	header('Location: '.$urlprincipal.$_POST['idOri'].'/'.$_POST['id_anuncio'].'/coincidencias');
	exit;
...
if ($_POST['botonesmio']) {
	...
	header('Location: '.$urlprincipal.$_POST['idOri'].'/'.$_POST['id_anuncio'].'/coincidencias/InformacionEnviada');
		
	exit;
	
}

$sql=" select * from anuncios where activo=1 and id_statusAnuncio=1  and id_anuncio=".$_GET[idOri]."
	 and id_cliente=".$_SESSION[id_clienteLof4]."  limit 1"; 
	$anuncioOriginal=$myvar->get_arreglo($sql);
	


$_SESSION[anunciosSimilares]='';

if($anuncioOriginal[0]['id_tipoAnuncio']==1){ //perdi
// los similares encontrados  o vistos 
	 
	 $sql='SELECT  a.* FROM similares s, anuncios a where  (s.id_anuncio="'.$anuncioOriginal[0]['id_anuncio'].'" and a.id_anuncio=s.id_anuncioSimilar 
	 and a.id_tipoAnuncio!=1 ) 
	 or (s.id_anuncioSimilar="'.$anuncioOriginal[0]['id_anuncio'].'"  and a.id_anuncio=s.id_anuncio and a.id_tipoAnuncio!=1 ) 
	 ';
	$sihay=$myvar->get_arreglo($sql);
								
	 $iban=0;
	 for($ss=0;$ss<count($sihay); $ss++){
		 	if($sihay[$ss][id_anuncio]==$anuncioOriginal[0]['id_anuncio']){
...
// los similares encontrados  o vistos 
	 ...
	 $sql='SELECT  s.* FROM similares s, anuncios a where  (s.id_anuncio="'.$anuncioOriginal[0]['id_anuncio'].'" and a.id_anuncio=s.id_anuncioSimilar and
	  a.id_tipoAnuncio=1 ) 
	 or (s.id_anuncioSimilar="'.$anuncioOriginal[0]['id_anuncio'].'"  and a.id_anuncio=s.id_anuncio and a.id_tipoAnuncio=1 ) 
	 ';
	$sihay=$myvar->get_arreglo($sql);
...
if(!$_GET[id])
$_GET[id]=$_SESSION['anunciosSimilares'][0];
...
function esmio(id,idOri){
		    ...
		    $.ajax({
                	data:  parametros,
	                url:   '<? echo $urlprincipal?>ajax-esmioCoincidencias.php',
	                type:  'get',
	                beforeSend: function () {
	                        $contenidoAjax.html();
	                },
...
<div class="menuAux2">
  <? 
	 if($anuncio[0][id_tipoAnuncio]==1){// perdid
			 ...
			 if(count($alguien)>0){?>
 				 <img src="<? echo $urlprincipal?>images/status/naranja.png"  border="0"> <a href="<? echo $urlprincipal?><? echo $_GET[id];?>/coincidencias">BOTON COINCIDENCIAS (<? echo count($alguien)?>)</a>
...
<? }else{?>
		<a  href='javascript:void(0);' onclick="esmio(<?php echo $anuncio[0][id_anuncio]; ?>,<?php echo $_GET[idOri]; ?>);"
...
<table width="100%" border="1" class="visor" cellspacing="0" height="600" cellpadding="0">
  <tr>
	...
	//if($bandera!=1){?>
            ...
            <a class="bt-3" href="<? echo $urlprincipal?><?php echo $_GET[idOri]?>/<?php echo $otrosproducto[0][id_anuncio]?>/coincidencias"> <img src="<? echo $urlprincipal?>images/FlechaV_Izq.png" class="img" border="0"></a>
            <?php } ?>
            </div>
            <div>
            
            <?php if($motrosproducto[0][id_anuncio]){ ?>
            <a class="bt-3" href="<? echo $urlprincipal?><?php echo $_GET[idOri]?>/<?php echo $motrosproducto[0][id_anuncio]; ?>/coincidencias"><img src="<? echo $urlprincipal?>images/FlechaV_Der.png" class="img" border="0"></a>
            <?php } ?>
            </div>
            </div>
    <? //}// if bandea?>
 ...
 <p>Agregar un comentario:</p>
             ...
             <input type="hidden" name="id_anuncio" id="id_anuncio" value="<? echo $anuncio[0][id_anuncio]?>"/>
