...
<h3> Editar nombre: </h3>
<label >Nombre: </label>
<input id="newNombre" name="nombre" value="<? echo $tipos3[0][name_Group] ?>" ></input>
...
(select id_clienteCliente,id_cliente from clientes_has_clientes where  id_cliente1=".$_SESSION[id_clienteLof4]." and id_statusSolicitud=1)";
    ...
    for($j=0; $j<count($tipos4);$j++){
     ...
     if($x == 0){
		...
		?>
	 <li> <label><input  type="checkbox" name="check_listEdit[]" id="amigosGrupo<? echo $tipos[$i][id_cliente]?>" value="<? echo $c[0][id_cliente] ?>">
     <?
